...
import type { IngestionConfig } from "@/types/rag-types";
    ...
    try {
      for (const file of selectedFiles) {
        ...
        const { data, error } = await apiCall<{
          documentId?: string; status?: string; totalChunks?: number; totalCharacters?: number;
        ...
        setUploadProgress(prev => ({ ...prev, [file.name]: isReady ? 'done' : 'processing' }));
      ...
      const processingCount = uploadedDocs.filter(d => d.status === 'processing').length;

      if (processingCount > 0) {
        toast({
          title: "Upload received",
          description: `${processingCount} document(s) still indexing — they'll appear as ready once processing completes.${readyCount > 0 ? ` ${readyCount} already indexed.` : ''}`,
        });
      } else {
        toast({
          title: "Success",
