...
// Selectable generation models. The `id` is sent to the ask-question API, which
// routes to Gemini or Claude. Gemini is the default.
const MODEL_OPTIONS = [
    ...
    try {
      ...
      if (!isResearchMode && attachedFiles.length > 0) {
        ...
        for (const file of attachedFiles) {
          ...
          const { data: uploadData, error: uploadError } = await apiCall(
            "upload-document",
            formData
          );
