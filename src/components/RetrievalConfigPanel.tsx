...
import type { RetrievalConfig, RetrievalConfigRecord } from "@/integrations/supabase/rag-types";
import {
...
export const RetrievalConfigPanel = ({
  ...
  const loadDbPresets = async () => {
    try {
      ...
      if (error) {
        // Table might not exist yet (before migration), fall back to client-side presets
        console.log('Using client-side retrieval presets (DB table not available)');
        setUseDbPresets(false);
        return;
      }

      if (data && data.length > 0) {
        setDbPresets(data as RetrievalConfigRecord[]);
